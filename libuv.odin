...
when ODIN_OS == .Windows {
    ...
    sockaddr        :: distinct windows.sockaddr
    addrinfo        :: distinct windows.ADDRINFOA
    uv_file         :: distinct c.int
    uv_os_sock_t    :: distinct windows.SOCKET
    uv_os_fd_t      :: distinct windows.HANDLE
    uv_pid_t        :: distinct c.int
    uv_uid_t        :: distinct u8
    uv_gid_t        :: distinct u8
    uv_thread_t     :: distinct windows.HANDLE
    uv_mutex_t      :: distinct windows.CRITICAL_SECTION
    uv_sem_t        :: distinct windows.HANDLE
    ...
    uv_pid_t        :: distinct i32
    uv_uid_t        :: distinct u32
    uv_gid_t        :: distinct u32
} else {
...
uv_udp_recv_cb      :: distinct proc "c" (handle: ^uv_udp_t, nread: c.ssize_t, buf: ^uv_buf_t, addr: ^sockaddr, flags: c.uint)
...
foreign uv {
    ...
    tcp_bind            :: proc (handle: ^uv_tcp_t, addr: ^sockaddr, flags: c.uint) -> c.int ---
    tcp_getsockname     :: proc (handle: ^uv_tcp_t, name: ^sockaddr, namelen: ^c.int) -> c.int ---
    tcp_getpeername     :: proc (handle: ^uv_tcp_t, name: ^sockaddr, namelen: ^c.int) -> c.int ---
    tcp_connect         :: proc (req: ^uv_connect_t, handle: ^uv_tcp_t, addr: ^sockaddr, cb: uv_connect_cb) -> c.int ---
    ...
    udp_bind            :: proc (handle: ^uv_udp_t, addr: ^sockaddr, flags: c.int) -> c.int ---
    udp_connect         :: proc (handle: ^uv_udp_t, addr: ^sockaddr) -> c.int ---
    udp_getpeername     :: proc (handle: ^uv_udp_t, name: ^sockaddr, namelen: ^c.int) -> c.int ---
    udp_getsockname     :: proc (handle: ^uv_udp_t, name: ^sockaddr, namelen: ^c.int) -> c.int ---
    ...
    udp_send            :: proc (req: uv_udp_send_t, handle: ^uv_udp_t, bufs: []uv_buf_t , nbufs: c.uint, addr: ^sockaddr, send_cb: uv_udp_send_cb) -> c.int ---
    udp_try_send        :: proc (handle: ^uv_udp_t, bufs: []uv_buf_t, nbufs: c.uint, addr: ^sockaddr) -> c.int ---
    ...
    getnameinfo         :: proc (loop: ^uv_loop_t, req: ^uv_getaddrinfo_t, getnameinfo_cb: uv_getnameinfo_cb , addr: ^sockaddr, flags: c.int) -> c.int ---
